<script lang="ts">
    ...
    const category:CategoryLoad = { name: "",slug: "",description: "",image:svelteCMS.defaults.asset }
    $: errorOnFields = {
    ...
    async function publish() {
        ...
        if(validated){
            // Send request
            const apiData:ApiCategoryCreateLoad = {...category,routeID}
            const apiResponse:ApiCategoryCreateData = await fetchPost("PUT",API_PATH,apiData)
            if(apiResponse.ok){
